 ...
 public class FtpClient
    {
        ...
        public bool WebUpload(HttpPostedFileBase file)
        {
            bool result = false;
            if (file!=null)
            {
                string url = Host.TrimEnd('/') + RemotePath +"UserImage/"+ file.FileName;
                FtpWebRequest request = CreateRequest(url, WebRequestMethods.Ftp.UploadFile);
                string allpath= "G:\\jpg\\"+file.FileName;

                //上传数据
                using (Stream rs = request.GetRequestStream())
                    
                    //获取
                using (FileStream fs = new FileStream(allpath, FileMode.Open))
                {
                    byte[] buffer = new byte[4096];//4K
                    int count = fs.Read(buffer, 0, buffer.Length);//每次从流中读4个字节再写入缓冲区
                    while (count > 0)
            ...
            throw new Exception(string.Format("本地文件不存在,文件路径:{0}", ""));
